...
class Key(object):
    ...
    def __hash__(self):
        return hash(' '.join((self.tonic, self.scale)))
#        return '~'.join((self.tonic, self.scale))
...
class PitchDistribution(object):
    ...
    @classmethod
    def from_stream(cls, str_data, sr):
        """
        ...
        C = ap.chromagram_from_stream(str_data, sr)
        ...
        cls.distribution = dist
        raw = copy.deepcopy(dist)
        dist.normalize()
        return dist, raw
    ...
    def to_dict(self):
        retdict = OrderedDict()
        for note in NOTES:
            retdict[note] = self.get_val(note)
        return retdict
    ...
    def concat(self, newdist):
        """
        Adds a dist to another dist
        """
        for note in NOTES:
            self.set_val(note, self.get_val(note) + newdist.get_val(note))
    ...
    def class_norm(self):
        ...
        if distribution_sum != 0:
            for note in NOTES:
                val = self.get_val(note)
                dist.set_val(note, val / float(distribution_sum))
        return dist
